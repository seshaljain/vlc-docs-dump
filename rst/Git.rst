`Git <https://git-scm.com/>`__ is a free and open source `version control system <wikipedia:Revision_control>`__. It is used by programmers to keep track of the different versions of the files composing a software.

Basic Git usage
---------------

If you are using **Windows**, please read the `Git Windows <Git_Windows>`__ page.

Getting VLC or x264 source code via Git
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``{{$}} git clone ``\ ```https://git.videolan.org/git/vlc.git`` <https://git.videolan.org/git/vlc.git>`__

Voilà! The full VLC history should be on your hard disk in vlc/.

``$ git clone ``\ ```https://git.videolan.org/git/x264.git`` <https://git.videolan.org/git/x264.git>`__

Voilà! The full x264 history should be on your hard disk in x264/.

If you want only the last 3 VLC revisions:

``$ git clone ``\ ```https://git.videolan.org/git/vlc.git`` <https://git.videolan.org/git/vlc.git>`__\ `` --depth 3``
...
``$ git clone ``\ ```https://git.videolan.org/git/vlc/vlc-2.2.git`` <https://git.videolan.org/git/vlc/vlc-2.2.git>`__
...
``$ git clone ``\ ```http://repo.or.cz/r/vlc.git`` <http://repo.or.cz/r/vlc.git>`__
...
You can also browse the sources via `gitweb <https://git.videolan.org/>`__.
...
*git-send-email* has built in support for sending e-mail through `SMTP <wikipedia:SMTP>`__. You'll need a command line similar to the following one:
...
You can use `msmtp <https://marlam.de/msmtp/>`__ to achieve this. Install it with tls and ssl activated and place the following config file (.msmtprc) in your home directory
...
The certificate for gmail can be found here: `EquifaxSecureCertificateAuthority.crt <https://www.geotrust.com/resources/extended-validation-ssl/certs/Equifax%20Secure%20Certificate%20Authority.crt>`__
...
``$ sudo -H cpan Net::``\ ```SMTP::SSL`` <SMTP::SSL>`__
...
``$ git commit ``\ \ `` --author "Name Surname <user@domain.com>"``
...
First make sure you have **read** our `Sending Patches <Sending_Patches>`__ page. And that you've **read** the `Check List <Sending_Patches#Check_List>`__.
...
To checkout that branch now see `#Fetching a remote branch <#Fetching_a_remote_branch>`__
...
``{{$}} git clone ``\ ```https://git.videolan.org/git/vlc/vlc-1.0.git`` <https://git.videolan.org/git/vlc/vlc-1.0.git>`__\ `` vlc-bugfix``

Add vlc.git as an additional remote:

``$ git remote add vlc-master ``\ ```https://git.videolan.org/git/vlc.git`` <https://git.videolan.org/git/vlc.git>`__
...
Go to http://repo.or.cz/w/vlc.git and click `fork <http://repo.or.cz/regproj.cgi?fork=vlc.git>`__. You will be able to publish your work there.
...
git has a great tool called `git-bisect <https://www.kernel.org/pub/software/scm/git/docs/git-bisect.html>`__ to help you to track a faulty commit. Imagine you are tracking a bug that is known to appear after 0.8.6 (assuming 0.8.6 is tagged):
...
will be able to track down the regression by itself. See `git-bisect Documentation <https://www.kernel.org/pub/software/scm/git/docs/git-bisect.html>`__.
...
-  You must have credentials to push commits into the repository. For other contributors, please read upper `Submitting patches to the vlc-devel or x264-devel <#Submitting_patches>`__ paragraph.
...
```https://git.videolan.org/git/vlc.git`` <https://git.videolan.org/git/vlc.git>`__
...
-  `Official Git Website <https://git-scm.com/>`__
-  `Git Tutorial <https://www.kernel.org/pub/software/scm/git/docs/tutorial.html>`__
-  `Switching from SVN to git using cogito <http://git.or.cz/course/svn.html>`__
-  `Very good Git guide <https://sourcemage.org/Git%20guide>`__
